...
function components_markersize(n)
    n ≤ components_markersize_threshold ? components_markersize_sparse :
    components_markersize_dense
end
...
function plot_components!(fg, axs, t, components; plottype = :lines, kwargs...)
    ...
    for (ax, component) in zip(axs, components)
        if plottype == :scatter
            scatter!(
                ax, t, component; markersize = components_markersize(length(t)), kwargs...)
        else
...
function plot_trajectory_cartesian(t, X, Y, Z, u; kwargs...)
    plot_components(t, (X, Y, Z, u); labels = (L"X", L"Y", L"Z", L"u"), kwargs...)
end

function plot_trajectory_cylindrical(t, R, Z, φ, u; kwargs...)
