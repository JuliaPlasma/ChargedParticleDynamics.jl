...
function plot_poincare_invariant_error(t, I; label = "I", kwargs...)
...
function plot_poincare_curves!(
        fg, ax, X, Y, Z; labels = nothing, plottype = :lines, kwargs...)
    @assert length(X) == length(Y) == length(Z)
    for i in eachindex(X)
        label = isnothing(labels) ? nothing : labels[i]
        if plottype == :scatter
            scatter!(ax, X[i], Y[i], Z[i]; markersize = poincare_surface_markersize,
                label = label, kwargs...)
        else
            lines!(ax, X[i], Y[i], Z[i]; linewidth = poincare_loop_linewidth,
                label = label, kwargs...)
        end
    end
    fg, ax
...
function plot_poincare_loop(
        t, X, Y, Z; nplot = 5, plottype = :lines, sigdigits = 4, kwargs...)
    steps = select_indices(eachindex(X), nplot)
    labels = ["t = $(round(t[n]; sigdigits = sigdigits))" for n in steps]
    fg, ax = plot_3d_figure(; kwargs...)
    plot_poincare_curves!(
        fg, ax, X[steps], Y[steps], Z[steps]; labels = labels, plottype = plottype)
    Legend(fg[1, 2], ax)
    fg, ax
end

function plot_poincare_surface(t, X, Y, Z; kwargs...)
    plot_poincare_loop(t, X, Y, Z; plottype = :scatter, kwargs...)
end
