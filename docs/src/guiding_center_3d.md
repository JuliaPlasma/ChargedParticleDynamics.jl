...
The equivalence is pinned by a test: `test/integration/structure.jl` spells Eq. (29) out directly from the `ElectromagneticFields` accessors for the three cartesian equilibria — including `QuadraticPotentials3d`, the one with a non-zero electrostatic potential — and checks both variants against it to round-off.
