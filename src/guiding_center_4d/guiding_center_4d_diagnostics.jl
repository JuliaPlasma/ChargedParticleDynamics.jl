# module Diagnostics
    ...
    import GeometricProblems.Diagnostics: compute_invariant, compute_one_form,
           compute_energy_error, compute_momentum_error, compute_error_drift

    export compute_energy, compute_toroidal_momentum,
           compute_energy_error, compute_momentum_error,
           compute_one_form, compute_error_drift
    ...
    compute_energy(t::TimeSeries, q::DataSeries, μ::Number) = compute_invariant(t, q, (t,q) -> hamiltonian(t, q, μ))
    compute_energy(sol::Solution, μ::Number) = compute_energy(sol.t, sol.q, μ)

    compute_energy_error(t::TimeSeries, q::DataSeries, μ::Number) = compute_energy_error(t, q, (t,q) -> hamiltonian(t, q, μ))
    compute_energy_error(sol::Solution, μ::Number) = compute_energy_error(sol.t, sol.q, μ)
